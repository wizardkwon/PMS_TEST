...
function adminCodeCheck(stringValue) {
	console.log("중복 검사 함수 출력됨");
	let adminCode = stringValue;
	...
	if (adminCode !== null) {
		if (adminCode === 'phone') {
			...
			alert('타 유저가 사용중인 전화번호입니다.');

		} else if (adminCode === 'account') {
			console.log("계좌 체크 출력됨");
			alert('타 유저가 사용중인 계좌번호입니다.');

		} else if (adminCode === 'sucess') {
			console.log("계좌 체크 출력됨");
			alert('수정이 완료되었습니다.');
